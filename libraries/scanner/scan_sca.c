...
static int GetZahl(Scanner *Handle, char StartZeichen, int StartK)
{  char AktZeichen;
   short k;
   ...
   do {
      ...
      AktZeichen = NextChar(Handle);
   } while (isdigit(AktZeichen));
   UngetChar(Handle, AktZeichen);
   return(k);
}

static int UngetZahl(Scanner *Handle, int EndK)
{  short k;

   k = EndK;
   while (isdigit(Handle->zk[k]))
   {
      UngetChar(Handle, Handle->zk[k]);
      k--;
   }
   return(k);
}
...
{  char AktZeichen;
   ...
   memset(Handle->zk, 0, SCAN_BLOCK_LAENGE);
   k = 0;
   k = GetZahl(Handle, StartZeichen, k);
   MerkZahl = Handle->i;
   AktZeichen = NextChar(Handle);
   if (AktZeichen == '.')
   {
      Handle->zk[k++] = AktZeichen;
      AktZeichen = NextChar(Handle);
      if (isdigit(AktZeichen))
      {
         k = GetZahl(Handle, AktZeichen, k);
         AktZeichen = NextChar(Handle);
         if (AktZeichen == '.')
         {
            Handle->zk[k++] = AktZeichen;
